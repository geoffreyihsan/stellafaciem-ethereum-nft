...
describe(CollectionConfig.contractName, function () {
  ...
  it("Before any sale", async function () {
    ...
    await (await contract.gift(1, await owner.getAddress())).wait();
    await (await contract.gift(1, await whitelistedUser.getAddress())).wait();
  ...
  it("Owner only functions", async function () {
    await expect(
      contract.connect(externalUser).gift(1, await externalUser.getAddress())
