...
async function main() {
  ...
  if (!(await (await contract.cost()).eq(preSalePrice))) {
    console.log(
      `Updating the token price to ${CollectionConfig.preSale.price} ETH...`
    );
  ...
  if (
    ...
    ).eq(CollectionConfig.preSale.maxMintAmountPerTx))
  ) {
    console.log(
      `Updating the max mint amount per TX to ${CollectionConfig.preSale.maxMintAmountPerTx}...`
    );
